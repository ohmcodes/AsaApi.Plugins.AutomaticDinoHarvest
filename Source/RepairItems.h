...
void RepairItemsCallback(AShooterPlayerController* pc, FString* param, int, int)
{
	...
	bool ignoreMaterials = AutomaticDinoHarvest::config["General"]["IgnoreInvRepairRequirements"].get<bool>();
	...
	{
		AsaApi::GetApiUtils().SendNotification(pc, FColorList::Green, 1.3f, 15.0f, nullptr, AutomaticDinoHarvest::config["Messages"].value("RepairItemsMSG", "All items has been repaired. {}").c_str(), pc->GetCharacterName().ToString());
